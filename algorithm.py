...
# you want to run algorithm with different parameters
__DEBUG = False
QI_LEN = 10
GL_K = 0
RESULT = []
...
def algorithm(data, k, relax=False, QI_num=-1):
    """
    Main function of algorithm, return result in tuple (result, (ncp, rtime)).
    ...
    relax: determine use strict or relaxed algorithm,
    Both algorithms split partition with binary split.
    In strict algorithm, lhs and rhs have not intersection.
    But in relaxed algorithm, lhs may be have intersection with rhs.
    """
    init(data, k, QI_num)
    result = []
    data_size = len(data)
